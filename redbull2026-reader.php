	...
	if(file_exists($file_csv)){
		...
		if(is_array($datas) && !empty($datas)){
			...
			foreach ($datas as $data) { 
				...
				$post['CUMPLIMIENTO_250_ROMBO'] = $data['CUMPLIMIENTO_250_ROMBO'] ?? "";
				$post['CUMPLIMIENTO_EDITIONS'] 	= $data['CUMPLIMIENTO_EDITIONS'] ?? "";
				$post['CUMPLIMIENTO_PONDERADO'] = $data['CUMPLIMIENTO_PONDERADO'] ?? "";
