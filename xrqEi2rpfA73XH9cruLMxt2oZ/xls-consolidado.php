...
$elements   = $objInforme->getIngresosConsolidado($grupo); // ajusta si usas $get
...
<table>
  ...
  <?php if (!empty($elements)): ?>
    <?php foreach ($elements as $r): 
      ...
      $primera     = htmlspecialchars((string)(sql2date($r->primera_visita ?? '')), ENT_QUOTES, 'UTF-8');
      $ultima      = htmlspecialchars((string)(sql2date($r->ultima_visita ?? '')), ENT_QUOTES, 'UTF-8');
      $ingresos    = htmlspecialchars((string)($r->ingresos ?? '0'), ENT_QUOTES, 'UTF-8');
    ...
    <tr>
        ...
        <td class="text"><?= $primera ?></td>
        <td class="text"><?= $ultima ?></td>
