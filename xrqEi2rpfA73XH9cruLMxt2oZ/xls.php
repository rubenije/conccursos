<?php
    ...
    if($post['opc'] == 'sendExport' && !empty($post['fecha_desde']) && !empty($post['fecha_hasta'])){
        $registro       = new registro();
        $elements       = $registro->getRegistroFilterAll($post);
    }else{
        $registro       = new registro();
        $elements       = $registro->getRegistroFilterAll($post);
    }
...
header('Content-Disposition: attachment;filename="'.date('Ymd-His').'-participantes.xls"');
