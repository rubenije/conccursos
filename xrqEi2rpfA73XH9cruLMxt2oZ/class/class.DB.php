...
//include_once(INCLUDE_PATH."class/class.upload.php");
...
define( 'DB_USER', 'root' ); // set database user
define( 'DB_PASS', 'root' ); // set database password
define( 'DB_NAME', 'aguamas' ); // set database name
...
class DB
{
    ...
    public function log_db_errors( $error, $query )
    {
        ...
        if( defined( 'SEND_ERRORS_TO' ) )
        {
            ...
            $headers .= 'From: LaPromoZero <no-reply@'.$_SERVER['SERVER_NAME'].'.cl>' . "\r\n";
    ...
    public function init(){
        if($_SERVER['HTTP_HOST'] == 'localhost:8888'){
            ...
            $this->dbUser = 'root';
            $this->dbPass = 'root';
            $this->dbName = 'conccursos';
            $this->dbPort = 8889;
        }else{
             $this->dbHost = 'localhost';
            $this->dbUser = 'conccursos_web';
            $this->dbPass = 'web.,2025';
            $this->dbName = 'conccursos_web';
            $this->dbPort = 3306;
        }   
    }
    ...
    public function __construct()
    {
        ...
        try {
            $this->link = new mysqli( $this->dbHost, $this->dbUser, $this->dbPass, $this->dbName, $this->dbPort );
    ...
    */
    ...
    public function uploadFileImage($name, $old_imagen){  

        $handle = new upload($_FILES[$name]);
        if ($handle->uploaded) {
            if(!empty($old_imagen)){
                unlink('../uploads/'.$old_imagen);
            }
            $handle->file_new_name_body   = uniqid("file_");
    ...
    public function createBG($name, $old_imagen){
        $handle = new upload($_FILES[$name]);
        ...
        if ($handle->uploaded) {
            ...
            $handle->file_new_name_body    = uniqid("bg-");
            $x  = $handle->image_src_x - 5;
            $y  = $handle->image_src_y - 5;
            $handle->image_crop = '0 '.$x.' '.$y.' 0';
            $handle->process('../uploads/');
    ...
    public function uploadFileArchivo($name, $old_imagen){  
    ...
    public function uploadFileBoleta($name, $old_imagen = ''){  
        pre($_FILES[$name]);
        $handle = new upload($_FILES[$name]);
        if ($handle->uploaded) {
            if(!empty($old_imagen)){
    ...
    public function uploadFileArchivoBases($name, $old_imagen){  
        ...
        if ($handle->uploaded) {
            ...
            $rand = date('YmdHis');
            $handle->file_new_name_body   = 'bases-legales-kpepsi-'.$rand;
            $handle->process('../uploads/');
            if ($handle->processed) {
                $upload = true;
                $handle->clean();
