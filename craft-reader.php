	...
	if(file_exists($file_csv)){
		...
		if(is_array($datas) && !empty($datas)){
			...
			foreach ($datas as $data) { 
				...
				$post['CUMPLIMIENTO_DUO_LN'] 		= $data['CUMPLIMIENTO_DUO_LN'] ?? "";
				$post['CUMPLIMIENTO_TORRES'] 		= $data['CUMPLIMIENTO_TORRES'] ?? "";
				$post['CUMPLIMIENTO_GUAYACAN'] 		= $data['CUMPLIMIENTO_GUAYACAN'] ?? "";
	...
	if(file_exists($file_csv)){
		...
		if(is_array($datas) && !empty($datas)){
			...
			foreach ($datas as $data) { 
				...
				$post['COB_CALAFATE_500'] 			= $data['COB_CALAFATE_500'] ?? "";
				$post['COB_TOROBAYO_500'] 		    = $data['COB_TOROBAYO_500'] ?? "";
				$post['COB_TORRES_PAINE_500'] 		= $data['COB_TORRES_PAINE_500'] ?? "";
				$post['COB_GUAYACAN_PAPAYA_500'] 	= $data['COB_GUAYACAN_PAPAYA_500'] ?? "";
