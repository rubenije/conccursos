<?php
	...
	$file_csv 	= 'GATORADE2026_DETALLE.csv';
	...
	if(file_exists($file_csv)){
		...
		if(is_array($datas) && !empty($datas)){
			...
			foreach ($datas as $data) { 
				...
				$post['COMPRA_GATORADE'] 		= $data['COMPRA_GATORADE'] ?? "";
				$post['CONCRETADO_750'] 		= $data['CONCRETADO_750'] ?? "";
				$post['CANTIDAD_CONC_1LT'] 		= $data['CANTIDAD_CONC_1LT'] ?? "";
				$post['VOL_VS_AA_GATORADE'] 	= $data['VOL_VS_AA_GATORADE'] ?? "";
				$post['VOL_VS_AA_750'] 			= $data['VOL_VS_AA_750'] ?? "";
