	...
	if(file_exists($file_csv)){
		...
		if(is_array($datas) && !empty($datas)){
			...
			foreach ($datas as $data) { 
				...
				$post['COB_BILZ'] 			= $data['COB_BILZ'] ?? "";
				$post['COB_PAP'] 		    = $data['COB_PAP'] ?? "";
				$post['COB_KEM'] 		    = $data['COB_KEM'] ?? "";
				$post['COB_LIMON_SODA'] 	= $data['COB_LIMON_SODA'] ?? "";
